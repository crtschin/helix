...
impl WatchFilter {
    ...
    /// Returns the innermost watched root containing `path` together with its
    /// ignore files, or the global ignores for a path outside every root.
    ///
    /// The list only ever holds the workspace plus the few roots added for LSP
    /// servers rooted outside it, so a scan costs less than a binary search and
    /// spares us an ordering invariant on `ignore_files`.
    fn ignores_for(&self, path: &Path) -> (&Path, &[Arc<Gitignore>]) {
        let innermost = self
            .ignore_files
            .iter()
        ...
        match innermost {
            Some(files) => (&files.root, &files.ignores),
            None => (Path::new(""), &self.global_ignores),
        }
    }
...
impl filesentry::Filter for WatchFilter {
    fn ignore_path(&self, path: &Path, is_dir: Option<bool>) -> bool {
        let (root, ignore_files) = self.ignores_for(path);
        if path == root {
            return false;
        }
...
mod tests {
    ...
    /// Build a filter over `roots`, each carrying a single gitignore pattern.
    fn filter(hidden: bool, roots: &[(&str, &str)]) -> WatchFilter {
        let ignore_files: Vec<_> = roots
            .iter()
            .map(|(root, pattern)| {
                ...
                IgnoreFiles {
                    ...
                    ignores: vec![Arc::new(builder.build().unwrap())],
                }
            })
            .collect();
        WatchFilter {
    ...
    fn ignore_path_rec_treats_ancestors_as_dirs() {
        // `target/` is a dir-only pattern: it matches the directory, not a file named target.
        let filter = filter(true, &[("/repo", "target/")]);
    ...
    #[test]
    fn ignores_apply_to_the_innermost_root() {
        // deliberately not in sorted order: the lookup must not depend on it
        let filter = filter(
            true,
            &[("/repo", "target/"), ("/other/dep", "node_modules/")],
        );

        assert!(filter.ignore_path_rec(Path::new("/repo/target/foo.rs"), Some(false)));
        assert!(filter.ignore_path_rec(Path::new("/other/dep/node_modules/x.js"), Some(false)));
        // one root's patterns must not leak into another
        assert!(!filter.ignore_path_rec(Path::new("/repo/node_modules/x.js"), Some(false)));
        assert!(!filter.ignore_path_rec(Path::new("/other/dep/target/foo.rs"), Some(false)));
        // outside every root only the global ignores apply, which are empty here
        assert!(!filter.ignore_path_rec(Path::new("/elsewhere/target/foo.rs"), Some(false)));
        // a root itself is never ignored
        assert!(!filter.ignore_path(Path::new("/repo"), Some(true)));
        assert!(!filter.ignore_path_rec(Path::new("/other/dep"), Some(true)));
    }

    #[test]
    fn nested_roots_resolve_to_the_deepest_one() {
        // `add_root` mostly rejects roots covered by an existing one, but not always
        let filter = filter(true, &[("/repo", "target/"), ("/repo/vendor", "dist/")]);
        assert!(filter.ignore_path_rec(Path::new("/repo/vendor/dist/x.js"), Some(false)));
    }

    #[test]
    fn hidden_config_hides_dotfiles() {
        let hide = filter(true, &[("/repo", "target/")]);
        assert!(hide.ignore_path(Path::new("/repo/.foo"), Some(false)));
        // the hardcoded whitelist and the vcs rules outrank the hidden check
        assert!(!hide.ignore_path(Path::new("/repo/.helix"), Some(true)));
        assert!(!hide.ignore_path(Path::new("/repo/.git"), Some(true)));

        let show = filter(false, &[("/repo", "target/")]);
        assert!(!show.ignore_path(Path::new("/repo/.foo"), Some(false)));
    }
}
